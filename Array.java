...
public class Array {
	...
	public static void dfs_three_sum(int[] a, List<List<Integer>> result, int left, List<Integer> now, int sumnumber,
			int index) {
	...
	// public static void dfs_sum(int s[],List<List<Integer>> result,List<Integer>
	// temp,int target,int index)
	// {
	 ...
	 *            最终结果集合
	 */
	private void recusionSolution(int[] nums, int k, int sum, int index, ArrayList<Integer> tempList,
		...
		if (k == 2) {
			...
			while (first < tail) {
				if (nums[first] + nums[tail] == sum) {
					...
					if (isMatch && first_last_value == nums[first]) {
						...
						tail--;
						continue;
					} else if (isMatch && tail_last_value == nums[tail] && first_last_value == nums[first]) {
						first++;
						tail--;
						continue;
					}
			...
			for (int i = index; i < nums.length - k; ++i) {
				...
				tempList.add(nums[i]);
				recusionSolution(nums, k - 1, sum - nums[i], i + 1, tempList, resultList);
	...
	// kmp算法
//	public int[] calPrefix(char p[]) {
//		int n = p.length;
//		int[] prefix = new int[n];
//		//len menas the smallest shared repeat str length!!!
//		// or len means the next index to be compare to the p[i],whcih means 0 to len-1 are
//		// already huiwen
//		// if equals means prefix[i]=++len;
//		int i=0;
//		prefix[0]=0;
//		int len=0;
//		while(i<n)
...
//	}
   //kmp
	...
	//假设next已经将prefix向右移动并且添加首位为-1 来使用next
	public void comparestr(String text,String pattern)
	{
